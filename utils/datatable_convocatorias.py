...
class DataTableConv(BoxLayout):
    def __init__(self, table='', callback=None, **kwargs):  # Añadido parámetro callback
        ...
        for r in range(rows_len):
            ...
            # Revisar el status y definir el texto del botón "Acción"
            status_text = 'Cerrar' if table['status'][r] == 'Abierta' else 'Abrir'
            action_button = {'text': status_text, 'size_hint_x': 0.2, 'size_hint_y': None, 'height': 30, 'bcolor': (.75, .12, .25, 1), 'index': r, 'callback': callback}
            table_data.append(action_button)
